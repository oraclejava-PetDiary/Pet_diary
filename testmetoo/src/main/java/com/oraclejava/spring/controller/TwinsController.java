...
public class TwinsController {
	...
	public String editu(@Valid Twins twins, 
		...
		if (check) {
			...
			attributes.addFlashAttribute("fail", "업데이트 실패! 관리자에게 문의주십시오");
		}
	...
	public String create_c(@Valid Twins twins, BindingResult bindingResult) {
		logger.info("create_c");
		
		Twins id_check=twinsDao.getTwinsOne(twins.getId());
		if(id_check!=null) {//기존의 아이디
			bindingResult.rejectValue("id","error.dupid","아이디가 중복되었습니다.다른id를 선택하세요");
